...
TVSERVER_BINARIES := usr/lib/libtvclient.so usr/lib/libtv.so usr/bin/tvservice usr/bin/tvtest usr/bin/hdmiin-demo usr/bin/streambox-tv
...
tvserver-configure:
	$(MAKE) -f Makefile -C $(TVSERVER_PROJDIR) $(TVSERVER_MAKE_OPTIONS) configure || true

tvserver-build:
	(PATH=$(CROSSTOOLS_BIN):$(PATH) \
	$(MAKE) -f Makefile -C $(TVSERVER_PROJDIR) $(TVSERVER_MAKE_OPTIONS) all)

tvserver-install:
	$(MAKE) -f Makefile -C $(TVSERVER_PROJDIR) DESTDIR=$(TVSERVER_INSTDIR) $(TVSERVER_MAKE_OPTIONS) DESTDIR=$(ROKU_STAGING_DIR)/$(TVSERVER_DESTDIR) install

tvserver-clean:
	$(MAKE) -f Makefile -C $(TVSERVER_PROJDIR) $(TVSERVER_MAKE_OPTIONS) clean || true

tvserver-distclean:
	$(MAKE) -f Makefile -C $(TVSERVER_PROJDIR) $(TVSERVER_MAKE_OPTIONS) DESTDIR=$(ROKU_STAGING_DIR)/$(TVSERVER_DESTDIR) distclean || true
